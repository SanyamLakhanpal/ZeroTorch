...
"""
- you can wrap with ddp as model =  DistributedDataParallel(model, device_ids=[local_rank])
- the forward  pass with data will be same model(data)
- but if we are using encoder or decoder, than instead of mode.encode --> model.module.encode()

- save the state only in global_rank == 0
    ...
    for epoch in range(num_epochs):
        for data, labels in data_loader:
            if (step_number + 1) % 100 != 0 and not last_step:  # Accumulate gradients for 100 steps
                """
                - Gradient accumulation is done for evey 10 steps
                """
